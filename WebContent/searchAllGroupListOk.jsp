			...
			<ul class="pagination">
					<c:if test="${page > block }">
						<li class="paginate_button previous">
							<a href="searchAllGroupList.do?page=1&area=${area }&interest=${interest}">◀◀</a></li>
						<li> <a href="searchAllGroupList.do?page=${startBlock - 1 }&area=${area }&interest=${interest }">◀</a></li>
					</c:if>
					
					<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
						<c:if test="${i == page }">
							<li class="active"> <a href="searchAllGroupList.do?page=${i }&area=${area }&interest=${interest }">${i }</a></li>
						</c:if>
						
						<c:if test="${i != page }">
							<li> <a href="searchAllGroupList.do?page=${i }&area=${area }&interest=${interest }">${i }</a></li>
						</c:if>
					</c:forEach>
					
					<c:if test="${endBlock < allPage }">
						<li> <a href="searchAllGroupList.do?page=${endBlock + 1 }&area=${area }&interest=${interest }">▶</a></li>
						<li class="paginate_button next">
							<a href="searchAllGroupList.do?page=${allPage }&area=${area }&interest=${interest }">▶▶</a></li>
					</c:if>
			</ul> 
		</div>
